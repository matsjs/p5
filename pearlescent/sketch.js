...
function rorschach (width, height) {
  for (let x = 0; x < width; x++) {
    for (let y = 0; y < height; y++) {
      ...
      // }
      // 
      const xGrad = Math.abs(OSNoise.noise2D((x-1)*scale,(y)*scale) - OSNoise.noise2D((x+1)*scale,y*scale))
      const yGrad = Math.abs(OSNoise.noise2D((x)*scale,(y-1)*scale) - OSNoise.noise2D((x)*scale,(y+1)*scale))
