...
function setup() {
  ...
  frameRate(50)
  pixelDensity(1);
  noise_func = new OpenSimplexNoise(2);
  createCanvas(CANVAS_W, CANVAS_H)
  background(0)
  // draw_map_dots(pos * TIMESCALE)
}
...
function new_position(x, y, plane_angle, offset) {
  return createVector(Math.round(x + plane_angle.x*offset), Math.round(y + plane_angle.y*offset))
}
...
function create_displacement_map(pos) {
  ...
  for (let x = 0; x < CANVAS_W; x++) {
    ...
    for (let y = 0; y < CANVAS_H; y++) {
      displacement_map[x].push(noise_func.noise4D(x*SCALE, y*SCALE, pos ? pos : 0))
    }
  }
...
function displace_uniform_light(displacement_map) {
  ...
  for (let x = 1; x < displacement_map.length -1; x++) {
    for (let y = 1; y < displacement_map[0].length -1; y++) {
      ...
      if (new_x >= 0 && new_x <= CANVAS_W-1 && new_y >= 0 && new_y <= CANVAS_H-1) {
        refraction_map[new_x][new_y] += 0.02
      }
    }
  }
...
function draw_map_dots(pos) {
  // refracted_map = create_displacement_map(pos)
  refracted_map = displace_uniform_light(create_displacement_map(pos))
